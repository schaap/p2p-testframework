...
class saveisseeder(processor):
    """
    A very simple processor that simply saves whether an execution was a seeder or not.
    ...
    - 'isSeeder_{0}'.format( execution.getNumber() )
    -- contains one line: "YES" if the execution was a seeder, otherwise "NO"
    """
    ...
    def processLogs(self, baseDir, outputDir):
        ...
        for e in self.scenario.getObjects( 'execution' ):
            f = open( os.path.join( outputDir, 'isSeeder_{0}'.format( e.getNumber() ) ), 'w' )
            if e.isSeeder():
                f.write( "YES" )
            else:
                f.write( "NO")
