...
class savefiles(processor):
    """
    A very simple processor that simply saves the names of the files with which each execution was done.
    ...
    - 'files_{0}'.format( execution.getNumber() )
    -- contains one line per file: the name of the file as reported by the file object's .name
    """
    ...
    def processLogs(self, baseDir, outputDir):
        ...
        for e in self.scenario.getObjects( 'execution' ):
            f = open( os.path.join( outputDir, 'files_{0}'.format( e.getNumber() ) ), 'w' )
            for f_ in e.files:
                f.write( "{0}\n".format( f_.name ) )
            f.close()
